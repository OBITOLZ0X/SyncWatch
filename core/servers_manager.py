...
class ServersManager:
    ...
    def scan_servers_parallel(self, servers: List[Dict],
                              progress_callback=None) -> List[Dict]:
        """Scan ALL servers in parallel using threads, measuring ping & querying room counts.

        Much faster than scan_servers_sequential when there are many servers.
        Calls progress_callback(current, total, server_dict, status_msg)
        after each server is checked.

        Returns the sorted server list (same sort order as scan_servers_sequential).
        """
        total = len(servers)
        if total == 0:
            self._cache = []
        ...
        # Thread-safe counter and lock for progress updates
        _completed_lock = threading.Lock()
        _completed_count = [0]  # mutable list for closure
        _results = [None] * total

        def _check_one(idx: int, srv: Dict):
            """Measure ping for a single server (runs in its own thread)."""
            url = srv.get("url", "")
            host_ip = srv.get("host", "")
            country = srv.get("country", "Unknown")
            country_code = srv.get("country_code", "??")
            ...
            # Emit progress (thread-safe via lock)
            with _completed_lock:
                _completed_count[0] += 1
                current = _completed_count[0]

            if progress_callback:
                progress_callback(
                    current, total,
                    {**srv},
                    f"Checking server {current}/{total}: {display_ip} ({country_code})"
                )

        # ── Step 1: Launch all ping threads in parallel ──
        threads = []
        for i, srv in enumerate(servers):
            t = threading.Thread(target=_check_one, args=(i, srv), daemon=True)
            threads.append(t)
            t.start()
        ...
        # ── Step 2: Query room counts for online servers (parallel) ──
        online_servers = [srv for srv in _results if srv and srv.get("status") == "online"]
        if online_servers:
            # _query_room_counts already uses threading
            self._query_room_counts(online_servers)

        # Rebuild full list preserving all servers (including offline)
        final_servers = [r for r in _results if r is not None]

        # Sort: online with rooms first (by rooms asc, then ping asc),
        # then online without rooms (by ping asc), then offline
        final_servers.sort(key=lambda s: (
            0 if s.get("status") == "online" and isinstance(s.get("rooms"), int) and s["rooms"] > 0 else
            1 if s.get("status") == "online" else 2,
            s.get("rooms", 9999) if isinstance(s.get("rooms"), int) else 9999,
            s.get("ping_ms", 9999) if isinstance(s.get("ping_ms"), (int, float)) and s.get("ping_ms") >= 0 else 9999,
        ...
        self._cache = list(final_servers)
        self._cache_time = time.time()
