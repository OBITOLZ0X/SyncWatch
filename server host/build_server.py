...
# Handle both file names: "server host.py" (with space, legacy Windows) and "server.py"
_candidates = [os.path.join(SCRIPT_DIR, "server host.py"), os.path.join(SCRIPT_DIR, "server.py")]
ENTRY = next((p for p in _candidates if os.path.isfile(p)), _candidates[1])
