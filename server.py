...
class SyncWatchServer:
    ...
    def encrypt_servers_data(self, raw_json: str) -> str:
        """Encrypt server list JSON using the sync server crypto."""
        try:
            from core.token_utils import server_data_encrypt
            return server_data_encrypt(raw_json)
        except ImportError:
            log.warning("server_data_encrypt not available, using plaintext")
            return raw_json

    def decrypt_servers_data(self, encrypted: str) -> str:
        """Decrypt server list JSON."""
        try:
            from core.token_utils import server_data_decrypt
            return server_data_decrypt(encrypted)
        except ImportError:
            return encrypted
