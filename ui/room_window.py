...
class RoomWindow(QWidget):
    ...
    def _player_name(self) -> str:
        """Return 'VLC' or 'MPV' depending on controller type."""
        from core.mpv_controller import MPVController
        return "MPV" if isinstance(self._vlc, MPVController) else "VLC"

    def _on_vlc_ready(self):
        self._sys_msg(f"{self._player_name} is ready.", styles.GREEN)
    ...
    def _on_vlc_closed(self):
        self._sys_msg(f"{self._player_name} was closed.", styles.RED)
        self._file_loaded = False
        self._is_ready = False
        self._refresh_ready_btn()
    ...
    def _load_file(self):
        ...
        if not self._vlc.connected:
            ...
            if not ok:
                self._sys_msg(f"Failed to launch {self._player_name}. Check Settings.", styles.RED)
                return
