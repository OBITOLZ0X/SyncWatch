...
class ScanProgressWidget(QFrame):
    """Inline widget showing only a spinner and 'Scanning servers' text."""
    ...
    def update_progress(self, current: int, total: int, srv: dict, status_msg: str):
        """Stub — no progress details shown to keep UI clean."""
        pass

    def reset_progress(self):
        """Stub — no progress details to reset."""
        pass
...
class MainWindow(QWidget):
    ...
    def _refresh_server_list(self, force: bool = False):
        ...
        def _worker():
            ...
            # Step 2: parallel scan with progress (much faster)
            def _progress_cb(current, total, srv, msg):
                self._sig_scan_progress.emit(current, total, srv, msg)

            self._servers_manager.scan_servers_parallel(servers, progress_callback=_progress_cb)
    ...
    def _panel_settings(self) -> QWidget:
        ...
        # ── Player selection ──
        player_sel_lbl = QLabel("PLAYER")
        player_sel_lbl.setObjectName("fieldLabel")
        lay.addWidget(player_sel_lbl)

        player_sel_row = QHBoxLayout()
        player_sel_row.setSpacing(8)
        self.s_player_vlc = QPushButton("VLC")
        self.s_player_vlc.setObjectName("accentBtn")
        self.s_player_vlc.setCursor(Qt.PointingHandCursor)
        self.s_player_vlc.setMinimumHeight(34)
        self.s_player_vlc.clicked.connect(lambda: self._set_player("vlc"))
        player_sel_row.addWidget(self.s_player_vlc)

        self.s_player_mpv = QPushButton("MPV")
        self.s_player_mpv.setCursor(Qt.PointingHandCursor)
        self.s_player_mpv.setMinimumHeight(34)
        self.s_player_mpv.clicked.connect(lambda: self._set_player("mpv"))
        player_sel_row.addWidget(self.s_player_mpv)
        lay.addLayout(player_sel_row)
        lay.addSpacing(10)
        ...
        # ── MPV path ──
        mpv_row = QHBoxLayout()
        self.s_mpv = QLineEdit()
        self.s_mpv.setPlaceholderText("Auto-detected or browse manually")
        mpv_row.addWidget(self.s_mpv)
        browse_mpv = QPushButton("Browse")
        browse_mpv.setObjectName("accentBtn")
        browse_mpv.setCursor(Qt.PointingHandCursor)
        browse_mpv.clicked.connect(self._browse_mpv)
        mpv_row.addWidget(browse_mpv)
        lbl_mpv = QLabel("MPV PATH")
        lbl_mpv.setObjectName("fieldLabel")
        lay.addWidget(lbl_mpv)
        lay.addLayout(mpv_row)
        lay.addSpacing(12)
    ...
    def _load_settings(self):
        ...
        self.s_mpv.setText(s.value("mpv_path", ""))
        self._player_choice = s.value("player", "vlc")
        self._set_player(self._player_choice)
        ...
        # Restore last room name on Host panel
        self.h_room_server.setText(s.value("last_room_name", ""))
    ...
    def _save_settings(self):
        ...
        s.setValue("mpv_path", self.s_mpv.text().strip())
        s.setValue("player", self._player_choice)
        ...
        # Persist last room name so the field is pre-filled next time
        room_name = self.h_room_server.text().strip()
        if room_name:
            s.setValue("last_room_name", room_name)
    ...
    def _browse_mpv(self):
        ...
        if path:
            self.s_mpv.setText(path)

    def _set_player(self, choice: str):
        """Set the active player (vlc or mpv) and update button highlights."""
        self._player_choice = choice
        if choice == "vlc":
            self.s_player_vlc.setObjectName("accentBtn")
            self.s_player_mpv.setObjectName("plainBtn")
        else:
            self.s_player_vlc.setObjectName("plainBtn")
            self.s_player_mpv.setObjectName("accentBtn")
        for btn in (self.s_player_vlc, self.s_player_mpv):
            btn.style().unpolish(btn)
            btn.style().polish(btn)
    ...
    def _open_room_validated(self, *, is_host, username, room_name,
        ...
        player_choice = getattr(self, '_player_choice', 'vlc')
        if player_choice == 'mpv':
            mpv_path = self.s_mpv.text().strip() or None
            player = MPVController(mpv_path=mpv_path)
        else:
            vlc_path = self.s_vlc.text().strip() or None
            player = VLCController(vlc_path=vlc_path)
        client = SyncClient()
        ...
        self._room_window = RoomWindow(
            client=client, vlc=player, is_host=is_host,
