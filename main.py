...
class SplashWindow(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.drawText(ver_rect, Qt.AlignCenter, "v3.7.2")
...
# Module-level state dict — accessible to ui/main_window.py via import main
_prefetch_state = {
    "prefetch_servers": None,
    "prefetch_done": False,
    "prefetch_current": 0,
    "prefetch_total": 0,
    "prefetch_msg": "",
...
def _prefetch_servers(state: dict):
    """Background: fetch + scan servers and store results in *state*."""
    log = logging.getLogger("prefetch")
    try:
        log.info("Starting pre-fetch of servers during splash...")
        mgr = ServersManager()
        servers = mgr.fetch_servers(force=True)
        if servers:
            def _progress_cb(current, total, srv, msg):
                state["prefetch_current"] = current
                state["prefetch_total"] = total
                state["prefetch_msg"] = msg
            mgr.scan_servers_sequential(servers, progress_callback=_progress_cb)
        state["prefetch_servers"] = servers
        state["prefetch_done"] = True
        log.info("Pre-fetch of %d servers complete.", len(servers))
    except Exception as exc:
        log.warning("Pre-fetch failed: %s", exc)
        state["prefetch_servers"] = []
        state["prefetch_done"] = True
        state["prefetch_error"] = str(exc)
...
def main():
    ...
    # ── Start pre-fetching servers in the background (during splash) ──
    # _prefetch_state is defined at module level above
    import threading
    pf_thread = threading.Thread(target=_prefetch_servers, args=(_prefetch_state,), daemon=True)
    pf_thread.start()

    # Keep refs alive to prevent GC
    _refs = {}

    def _show_main():
        # Pass pre-fetch results to main window
        kwargs = {}
        ps = _prefetch_state
        if ps.get("prefetch_servers") is not None:
            kwargs["prefetched_servers"] = ps["prefetch_servers"]
            kwargs["prefetch_done"] = ps.get("prefetch_done", True)
            kwargs["prefetch_current"] = ps.get("prefetch_current", 0)
            kwargs["prefetch_total"] = ps.get("prefetch_total", 0)
            kwargs["prefetch_msg"] = ps.get("prefetch_msg", "")
        window = MainWindow(**kwargs)
        window.show()
        _refs["window"] = window  # prevent GC
